...
daysWithHosts = dailyHosts.map(lambda (k, v): k).collect() #<FILL IN>
hosts = dailyHosts.map(lambda (k, v): v).collect() #<FILL IN>
...
dayAndHostTuple = access_logs.map(lambda log: (log.date_time.day, log.host)) #<FILL IN>

groupedByDay = dayAndHostTuple.groupByKey().mapValues(len) #<FILL IN>

sortedByDay = groupedByDay.sortByKey() #<FILL IN>

avgDailyReqPerHost = (sortedByDay.join(dailyHosts).mapValues(lambda v: v[0]/v[1])
                     #.map(lambda (k, v): (k, v[0]/v[1]))
                     .sortByKey().cache()) #<FILL IN>)
...
daysWithAvg = avgDailyReqPerHost.map(lambda (k, v): k).collect() #<FILL IN>
avgs = avgDailyReqPerHost.map(lambda (k, v): v).collect() #<FILL IN>
...
badEndpointsCountPairTuple = badRecords.map(lambda log: (log.endpoint, 1)) #<FILL IN>

badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(lambda a, b: a + b) #<FILL IN>

badEndpointsTop20 = badEndpointsSum.takeOrdered(20, lambda (k, v): -v) #<FILL IN>
...
errHostsCountPairTuple = badRecords.map(lambda log: (log.host, 1)) #<FILL IN>

errHostsSum = errHostsCountPairTuple.reduceByKey(lambda a, b: a + b) #<FILL IN>

errHostsTop25 = errHostsSum.takeOrdered(25, lambda (k, v): -v) #<FILL IN>
...
errDateCountPairTuple = badRecords.map(lambda log: (log.date_time.day, 1)) #<FILL IN>

errDateSum = errDateCountPairTuple.reduceByKey(lambda a, b: a + b) #<FILL IN>

errDateSorted = (errDateSum.sortByKey()
                 ).cache()
                 #<FILL IN>)

errByDate = errDateSorted.take(30) #<FILL IN>
...
daysWithErrors404 = errDateSorted.map(lambda (k, v): k).collect() #<FILL IN>
errors404ByDay = errDateSorted.map(lambda (k, v): v).collect() #<FILL IN>
...
topErrDate = errDateSorted.takeOrdered(5, lambda (k, v): -v) #<FILL IN>
...
hourCountPairTuple = badRecords.map(lambda log: (log.date_time.hour, 1)) #<FILL IN>

hourRecordsSum = hourCountPairTuple.reduceByKey(lambda a, b: a + b) #<FILL IN>

hourRecordsSorted = (hourRecordsSum.sortByKey()
                     .cache())
                     #<FILL IN>)

errHourList = hourRecordsSorted.take(30) #<FILL IN>
...
hoursWithErrors404 = hourRecordsSorted.map(lambda (k, v): k).collect() #<FILL IN>
errors404ByHours = hourRecordsSorted.map(lambda (k, v): v).collect() #<FILL IN>
