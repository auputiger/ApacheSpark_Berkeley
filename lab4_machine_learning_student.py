...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    RatingsAvg = sum(Ratings) * 1.0 / RatingsCount
    return (MovieID, (RatingsCount, RatingsAvg)) #<FILL IN>
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(getCountsAndAverages) #<FILL IN>
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda s: s[2] > 500) #<FILL IN>
...
validationForPredictRDD = validationRDD.map(lambda (UserID, MovieID, Rating): (UserID, MovieID)) #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
myModel = ALS.train(trainingRDD, bestRank, seed = seed, iterations = iterations, lambda_ = regularizationParameter) #<FILL IN>
testForPredictingRDD = testRDD.map(lambda (UserID, MovieID, Rating):(UserID, MovieID)) #<FILL IN>
predictedTestRDD = myModel.predictAll(testForPredictingRDD) #<FILL IN>
...
trainingAvgRating = trainingRDD.map(lambda (UserID, MovieID, Rating): float(Rating)).mean() 
                    #.reduce(lambda a, b: a + b) #<FILL IN>

print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (UserID, MovieID, Rating): (UserID, MovieID, trainingAvgRating)) #<FILL IN>
...
myRDD = (moviesRDD.join(movieIDsWithAvgRatingsRDD)
         .map(lambda (movieID, (title, (count, avg))): (avg, (title, count, movieID)))
         .filter(lambda (avg, (title, count, movieID)): count>500).sortByKey(False)
         )
#.map(lambda (a, (b, (c,d))): (d, (b, c, a))).filter(lambda (d, (b, c, a)): c>500).sortByKey(False))
print myRDD.take(200)
...
myRatedMovies = [
     #<FILL IN>
(myUserID, 318, 5), #u'Shawshank Redemption, The (1994)'
(myUserID, 527, 5), #u"Schindler's List (1993)"
(myUserID, 858, 4), #u'Godfather, The (1972)'
(myUserID, 1221, 3), #u'Godfather: Part II, The (1974)'
(myUserID, 296, 4), #u'Pulp Fiction (1994)'
(myUserID, 1337, 5), #u'Saving Private Ryan (1998)'
(myUserID, 593, 4), #u'Silence of the Lambs, The (1991)'
(myUserID, 2858, 3), #u'American Beauty (1999)'
(myUserID, 356, 5), #u'Forrest Gump (1994)'
(myUserID, 590, 5), #u'Dances with Wolves (1990)'
(myUserID, 1704, 3), #u'Good Will Hunting (1997)'
(myUserID, 2692, 2), #u'Run Lola Run (Lola rennt) (1998)'
(myUserID, 364, 5), #u'Lion King, The (1994)'
(myUserID, 2959, 3), #u'Fight Club (1999)'
(myUserID, 2706, 1), #u'American Pie (1999)'
(myUserID, 1307, 5), #u'When Harry Met Sally... (1989)'
(myUserID, 1035, 4), #u'Sound of Music, The (1965)'
(myUserID, 1961, 4) #u'Rain Man (1988)'
...
# TODO: Replace <FILL IN> with appropriate code
trainingWithMyRatingsRDD = trainingRDD.union(myRatingsRDD) #<FILL IN>
...
myRatingsModel = ALS.train(trainingWithMyRatingsRDD, bestRank, 
                           seed = seed, iterations = iterations, lambda_ = regularizationParameter) #<FILL IN>)
...
# TODO: Replace <FILL IN> with appropriate code
predictedTestMyRatingsRDD = myRatingsModel.predictAll(testForPredictingRDD) #<FILL IN>
testRMSEMyRatings = computeError(testRDD, predictedTestMyRatingsRDD) #<FILL IN>
...
myUnratedMoviesRDD = (moviesRDD
                      ...
                      #.subtractByKey(myRatedMovies)
                      .filter(lambda (MovieID, Title): MovieID not in [s[1] for s in myRatedMovies])
                      .map(lambda (MovieID, Title): (myUserID, MovieID))
                      )
                      #.<FILL IN>)
# Use the input RDD, myUnratedMoviesRDD, with myRatingsModel.predictAll() to predict your ratings for the movies
predictedRatingsRDD = myRatingsModel.predictAll(myUnratedMoviesRDD) #<FILL IN>
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (MovieID, (count, avg)): (MovieID, count)) #<FILL IN>

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda (UserID, MovieID, predictedRating): (MovieID, predictedRating)) #<FILL IN>
...
ratingsWithNamesRDD = (predictedWithCountsRDD.join(moviesRDD)
                      .map(lambda (MovieID, ((predictedRating, count), title)): (predictedRating, title))
                      ) #<FILL IN>)
#print ratingsWithNamesRDD.take(3)
