...
def simpleTokenize(string):
    ...
    """
    return filter(lambda s: s != '', re.split(split_regex, string.lower()))  #<FILL IN>
...
def tokenize(string):
    ...
    """
    return filter(lambda s: s!='' and s not in stopwords, re.split(split_regex, string.lower()))  # <FILL IN>
...
amazonRecToToken = amazonSmall.map(lambda line: (line[0], tokenize(line[1]))) #<FILL IN>
googleRecToToken = googleSmall.map(lambda line: (line[0], tokenize(line[1]))) #<FILL IN>
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.flatMap(lambda (recordId, tokenizedValue): tokenizedValue).count() # <FILL IN>
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.sortBy(lambda (recordId, tokenizedValue): -len(tokenizedValue)).take(1) # <FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
corpusRDD = amazonRecToToken.union(googleRecToToken) #<FILL IN>
...
#N = corpus.map(lambda (recordId, tokenizedValue): recordId).distinct.count() #<FILL IN>
...
def idfs(corpus):
    ...
    """
    #N = corpus.map(lambda (recordId, tokenizedValue): recordId).distinct.count() #<FILL IN>
    N = corpus.map(lambda rec: rec[0]).distinct().count() #<FILL IN>
    uniqueTokens = corpus.flatMap(lambda rec: list(set(rec[1]))) #<FILL IN>
    tokenCountPairTuple = uniqueTokens.map(lambda token: (token, 1)) #<FILL IN>
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a + b) #<FILL IN>
    return (tokenSumPairTuple.map(lambda (k, v): (k, 1.0*N/v))) #<FILL IN>)
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens) #<FILL IN>
    #tfIdfDict = {}
    #for token in tfs:
    #    tfIdfDict[token] = tfs[token] * idfs[token]
    tfIdfDict = {k: tfs[k]*idfs[k] for k in tfs.keys()} #<FILL IN>
...
def dotprod(a, b):
    ...
    """
    return sum(a[k]*b[k] for k in a.keys() if b.has_key(k)) #<FILL IN>
...
def norm(a):
    ...
    """
    return math.sqrt(sum(a[k]**2 for k in a.keys())) #<FILL IN>
...
def cossim(a, b):
    ...
    """
    return dotprod(a, b) / (norm(a) * norm(b)) #<FILL IN>
...
def cosineSimilarity(string1, string2, idfsDictionary):
    ...
    """
    w1 = tfidf(tokenize(string1), idfsDictionary)  #(<FILL IN>)
    w2 = tfidf(tokenize(string2), idfsDictionary) #(<FILL IN>)
    return cossim(w1, w2)
...
def computeSimilarity(record):
    ...
    googleRec = record[0]
    amazonRec = record[1]
    googleURL = googleRec[0] #.map(lambda (k, v): k) #<FILL IN>
    amazonID =  amazonRec[0] #.map(lambda (k, v): k) #<FILL IN>
    googleValue = googleRec[1] #.map(lambda (k, v): v) #<FILL IN>
    amazonValue = amazonRec[1] #.map(lambda (k, v): v) #<FILL IN>
    cs = cosineSimilarity(googleValue, amazonValue, idfsSmallWeights) #(<FILL IN>, idfsSmallWeights)
    return (googleURL, amazonID, cs)

similarities = (crossSmall
                .map(computeSimilarity) #.<FILL IN>
                .cache())
...
def computeSimilarityBroadcast(record):
    ...
    googleRec = record[0]
    amazonRec = record[1]
    googleURL = googleRec[0] #<FILL IN>
    amazonID = amazonRec[0] #<FILL IN>
    googleValue = googleRec[1] #<FILL IN>
    amazonValue = amazonRec[1] #<FILL IN>
    cs = cosineSimilarity(googleValue, amazonValue, idfsSmallBroadcast.value) #(<FILL IN>, idfsSmallBroadcast.value)
    return (googleURL, amazonID, cs)
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (googleURL, amazonID, cs): (amazonID + " " + googleURL , cs)) #<FILL IN>)

trueDupsRDD = (sims
               .join(goldStandard)) #<FILL IN>)
#print trueDupsRDD.take(1)

trueDupsCount = trueDupsRDD.count() #<FILL IN>

avgSimDups = trueDupsRDD.map(lambda (k, v): v[0]).mean() #<FILL IN>

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard).filter(lambda (k, v): v[1] == None)) #<FILL IN>)

avgSimNon = nonDupsRDD.map(lambda(k, v): v[0]).mean() #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda rec: (rec[0], tokenize(rec[1]))) #<FILL IN>
googleFullRecToToken = google.map(lambda rec: (rec[0], tokenize(rec[1]))) #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
fullCorpusRDD = amazonFullRecToToken.union(googleFullRecToToken) #<FILL IN>
idfsFull = idfs(fullCorpusRDD)
idfsFullCount = idfsFull.count()
print 'There are %s unique tokens in the full datasets.' % idfsFullCount

# Recompute IDFs for full dataset
idfsFullWeights = idfsFull.collectAsMap() #<FILL IN>
idfsFullBroadcast = sc.broadcast(idfsFullWeights) #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (k, v): (k, norm(v))) #<FILL IN>
#amazonNorms = amazonWeightsRDD.map(lambda rec: (rec[0], norm(rec[1]))) #<FILL IN>
amazonNormsBroadcast = sc.broadcast(amazonNorms.collectAsMap()) #<FILL IN>
googleNorms = googleWeightsRDD.map(lambda (k, v): (k, norm(v)))
googleNormsBroadcast = sc.broadcast(googleNorms.collectAsMap()) #<FILL IN>
...
def swap(record):
    ...
    Returns:
        pair: ((ID, URL), token)
    """
    token = record[0] #<FILL IN>
    keys = record[1] #<FILL IN>
    return (keys, token)

commonTokens = (amazonInvPairsRDD.join(googleInvPairsRDD)
                .map(swap).groupByKey() #<FILL IN>
                .cache())
...
# TODO: Replace <FILL IN> with appropriate code
amazonWeightsBroadcast =  sc.broadcast(amazonWeightsRDD.collectAsMap()) #<FILL IN>
googleWeightsBroadcast =  sc.broadcast(googleWeightsRDD.collectAsMap()) #<FILL IN>
...
def fastCosineSimilarity(record):
    ...
    """
    amazonRec = record[0][0] #<FILL IN>
    googleRec = record[0][1] #<FILL IN>
    tokens = record[1] #<FILL IN>
    s = sum(amazonWeightsBroadcast.value[amazonRec][t] * googleWeightsBroadcast.value[googleRec][t] for t in tokens) #<FILL IN>
    value = s / (amazonNormsBroadcast.value[amazonRec] * googleNormsBroadcast.value[googleRec]) #<FILL IN>
    key = (amazonRec, googleRec)
    return (key, value)

similaritiesFullRDD = (commonTokens
                       .map(fastCosineSimilarity) #<FILL IN>
                       .cache())
